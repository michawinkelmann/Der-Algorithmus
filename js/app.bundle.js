...
// den dynamischen Charakter-Zustand (Bio nach NPC-Arc, Online-Status etc.)
// nachschiebt. Ohne Hook bleibt das Verhalten unverändert.
...
function getTrendingForCompose() {
...
function escapeHtml(s) {
  ...
  __M.getTrendingForCompose = getTrendingForCompose;
...
(function(){
  ...
  var maybeShowQueuedConcept = __M.maybeShowQueuedConcept;
...
function updateTopbar() {
  ...
  if (mb) mb.hidden = w < 2;
  const total = DATA?.weeks?.weeks?.length || 27;
  const fill = document.getElementById('week-progress-fill');
  if (fill) fill.style.width = Math.min(100, Math.round((w / Math.max(1, total - 1)) * 100)) + '%';
  updateDmBadge();
}
...
// NPCs verändern Bio (und manchmal Namen) je nach Spielverlauf — wird in
// getCharacter überlagert. Nur ein dünner Datenüberzug, kein Mutieren der
// Original-Charakter-Records.
